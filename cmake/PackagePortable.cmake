...
# Never recursively remove a caller-provided path outside this build's package
# directory. Resolve existing junctions before checking containment.
file(REAL_PATH "${INPUT_DIR}" package_input_root)
if(EXISTS "${STAGING_DIR}")
    file(REAL_PATH "${STAGING_DIR}" package_stage_root)
else()
    cmake_path(SET package_stage_root NORMALIZE "${STAGING_DIR}")
endif()
cmake_path(IS_PREFIX package_input_root "${package_stage_root}" NORMALIZE stage_inside_build)
if(NOT stage_inside_build OR package_stage_root STREQUAL package_input_root)
    message(FATAL_ERROR "Package staging must be a child of INPUT_DIR")
endif()
if(REQUIRE_NATIVE_NOTICES AND NOT EXISTS "${WEBRTC_NOTICES_DIR}/LICENSE.md")
    message(FATAL_ERROR "Native packages require a verified WebRTC SDK with generated dependency notices")
endif()
...
endforeach()
file(COPY_FILE "${SOURCE_DIR}/docs/portable-readme.md" "${STAGING_DIR}/README.md")
copy_package_file("${SOURCE_DIR}/LICENSE")
copy_package_file("${SOURCE_DIR}/third_party/ViGEm-NOTICE.txt")
copy_package_file("${SOURCE_DIR}/third_party/THIRD-PARTY-NOTICES.md")
copy_package_file("${SOURCE_DIR}/third_party/ICU-LICENSE.txt")
file(COPY "${SOURCE_DIR}/third_party/qt-6.10.3/" DESTINATION "${STAGING_DIR}/licenses/qt")
copy_package_file("${INPUT_DIR}/ViGEmClient-LICENSE.txt")
if(EXISTS "${WEBRTC_NOTICES_DIR}/LICENSE.md")
    file(COPY "${WEBRTC_NOTICES_DIR}/" DESTINATION "${STAGING_DIR}/licenses/webrtc")
endif()
if(EXISTS "${QT_SBOM_DIR}")
    file(COPY "${QT_SBOM_DIR}/" DESTINATION "${STAGING_DIR}/licenses/qt-sbom")
endif()
...
if(package_executables OR input_libraries)
    ...
    foreach(unresolved_runtime_dependency IN LISTS unresolved_runtime_dependencies)
        if(NOT unresolved_runtime_dependency MATCHES "^(api-ms-win|ext-ms)-")
            if(REQUIRE_NATIVE_NOTICES)
                message(FATAL_ERROR "Could not resolve native runtime dependency: ${unresolved_runtime_dependency}")
            else()
