...
- [ ] Connect HostPeerRegistry/IMediaPeer ownership and lifecycle action dispatch to the application signaling executor. Automatic capture cleanup now retries queue pressure without blocking Tick, and full registry stop joins capture first. The real four-peer proof covers failed-peer detachment and replacement. Preserve the rolling restart budget across reconnects; investigate reported restart timing before claiming responsive outage recovery.
...
- [ ] Consume shared PeerNegotiation futures in the application peer owner and route successful SDP through authenticated room messages. The owned signaling executor now runs real media proofs, with a separate no-pump offer/answer test. Integrate nonblocking completion dispatch; do not copy the diagnostic nested waits into application commands.
