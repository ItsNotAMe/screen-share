...
1. Validate NAT multi-viewer rooms with real computers using one sharer invite plus optional watcher response invites. Follow-ups after testing: per-viewer connection/health display, clearer per-viewer labels, optional per-viewer bandwidth policy, and better fallback UX for watchers whose NAT still needs a response invite.
