...
1. Finish refactoring the native engine into a reusable backend/core API that both the CLI and revamped UI can call directly. The UI live session path should stay in-process on a worker thread. Typed Share viewer status and Watch media activity have started; continue moving NAT hints, access-code failures, preview-close handling, and later media surfaces into typed events for Active Share/Watch Session screens.
