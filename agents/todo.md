...
- [ ] Connect HostPeerRegistry/IMediaPeer ownership and lifecycle action dispatch to the application signaling executor and capture failure cleanup. The real four-peer proof now uses the registry for ownership, queued restart, replacement and stop. Preserve the rolling restart budget across reconnects; investigate reported restart timing before claiming responsive outage recovery.
