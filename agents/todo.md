...
- [ ] Resolve rapid source-close StopCapture hang and hardware/recovery handle growth. The earlier Release crash was reproduced in unloaded GraphicsCapture.dll and mitigated by a one-time system-module pin; 100 full cycles pass. Carry proof recovery into the production session coordinator with callback-generation barriers; proof now has three rebuilds, cancellable backoff and per-device retirement across replacement generations.
