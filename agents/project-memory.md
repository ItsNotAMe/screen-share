...
- Signaling direction:
  ...
  - The UI default Internet path now uses the built-in Worker `https://screenshare-signaling.bit-yeet.workers.dev`: Share has Room/Port and copies a `screenshare-room-v1;room=...` link; Watch pastes that link but keeps its own local watch port, then launches `--watch PORT --signal-room ROOM` with the server supplied internally.
