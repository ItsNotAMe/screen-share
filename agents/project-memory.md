...
- Backend v2 is authorized on `refactor/backend-v2`; Gate A remains open. Use `refactor/PLAN.md`, `TODO.md`, the current section of `BUILD.md`, and the latest `CHECKPOINT-A.md` evidence. Native MF/WebRTC hardware/software, ADM/Opus, owned WGC capture, NV12 presentation and bounded injected-device recovery are proven locally. Content-addressed Debug/Release SDK exports and relocation pass; fresh media suites pass 14/14, application suites 9/9, and the native Release portable zip passes extracted CLI/UI/Windows startup with developer paths removed. Forty additional Release capture cycles passed (20 debugger / 20 normal), but the prior intermittent teardown failure remains unexplained. Shared v2 client-command validators and revision/generation tracking now have native/TypeScript fixtures; see refactor/ROOM-PROTOCOL.md. Next: server output schemas/fixtures, external latency baseline, lifecycle investigation and installer/distribution evidence. Actual driver removal and production session callback-generation barriers are not proven. Normal app media still uses the legacy backend; gaming latency remains unmeasured. Continue through substantial milestones without stopping at every intermediate commit.
