...
- Backend v2 refactor is authorized on `refactor/backend-v2`, with Checkpoint A in progress. Follow `refactor/PLAN.md` and `refactor/TODO.md`; evidence is in `refactor/CHECKPOINT-A.md`, commands in `refactor/BUILD.md`. User-authorized legacy UDP edits were archived under `build/pre-v2-legacy-edits` and removed. Native Debug/Release application suites pass 8 tests each. Hardware/audio-enabled WebRTC proof suites pass 12 each: WASAPI process-loopback through Opus, physical PCM lifecycle/mute/mono/stereo, MF software/hardware transmission, rate/lifecycle/burst handling, 500 ms missing-output fallback/quarantine, cancellation, GPU texture ownership and cached CPU readback. Run with `-Hardware -AudioDevice` (brief quiet tone). Owned live WGC snapshots now support GPU completion, fixed-output resize and hardware encoder import. WGC Close regression now passes after draining queued frames, flushing GPU commands and closing session before pool; six separate-process runs per configuration pass. Live hardware WGC-through-PeerConnection tests receive 60 frames with zero sender readbacks, Opus and three data channels. Receive presentation now uses owned CPU NV12, one replaceable pending frame, UI-thread GPU upload/rendering and nonblocking Present with a measured one-frame DXGI queue limit. Live receiver resize/visible content tests pass with no extra conversion/repack; hardware decoding remains pending. Source lifecycle now tracks WGC Closed with a shared flag/revoker, validates process/thread identity, suppresses minimized frames and permanently latches closure. Repeated minimize/restore, replacement-window and job-contained external process-exit tests pass. Device-loss errors are distinct at capture polling/completion boundaries, but automatic recovery is pending. Next: implement device recovery and forced HWND reuse stress coverage, and finish Gate A delivery evidence, plus broader audio mode/recovery validation. Output reports actual engine period and buffer capacity separately (10 ms / 1122 frames on this machine). Driver calls that never return cannot be preempted by the polling deadline. Private adapters/contracts are in `src/media/webrtc/`; normal app routing has not switched and gaming latency is unverified.
...
- Qt control UI is merged:
  ...
  - Live Share/Watch runs now go through `src/ui/QtSessionBackend.*` and `screenshare::ScreenShareSession`, so the UI calls the in-process session API on a worker thread instead of launching `ScreenShare.exe`.
