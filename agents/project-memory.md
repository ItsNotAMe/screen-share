...
- Backend v2 is authorized on `refactor/backend-v2`; Gate A remains open. Use `refactor/PLAN.md`, `TODO.md`, the current section of `BUILD.md`, and the latest `CHECKPOINT-A.md` evidence. Native MF/WebRTC hardware/software, ADM/Opus, owned WGC capture, NV12 presentation and bounded injected-device recovery are proven locally. Content-addressed Debug/Release SDK exports and relocation pass; fresh media suites pass 14/14, application suites 10/10, and the native Release portable zip passes extracted CLI/UI/Windows startup with developer paths removed. The intermittent Release crash was reproduced in unloaded GraphicsCapture.dll and mitigated by pinning the system module until process exit. 100 full Release cycles pass; rapid source-close StopCapture still hangs and adapter-filtered hardware/recovery handles now increase by about two per cycle (previously eight); isolated encoder and capture-device rebuild probes have stable handles. Hardware activation ownership now explicitly shuts down activated transforms; resource acceptance remains open. Capture-only Stop with the source open passes 100 cycles in each of Debug/Release with stable handle counts. Shared v2 command/server-event validators now have native/TypeScript fixtures, plus atomic native room/directory state caches; see refactor/ROOM-PROTOCOL.md. Next: rapid-close shutdown hang, hardware/recovery handle growth, external latency baseline and installer/distribution evidence. Actual driver removal and production session callback-generation barriers are not proven. Normal app media still uses the legacy backend; gaming latency remains unmeasured. Continue through substantial milestones without stopping at every intermediate commit.
