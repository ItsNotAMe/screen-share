...
- Backend v2 is authorized on `refactor/backend-v2`; Gate A remains open. Use `refactor/PLAN.md`, `TODO.md`, the current section of `BUILD.md`, and the latest `CHECKPOINT-A.md` evidence. Native MF/WebRTC hardware/software, ADM/Opus, owned WGC capture, NV12 presentation and bounded injected-device recovery are proven locally. Content-addressed Debug/Release SDK exports and relocation pass; media suites now pass 16/16, application suites 10/10, and the native Release portable zip passes extracted CLI/UI/Windows startup with developer paths removed. The intermittent Release crash was reproduced in unloaded GraphicsCapture.dll and mitigated by pinning the system module until process exit. 100 full Release cycles pass; capture-owner dispatcher integration now completes 100 rapid source-close cycles without the prior StopCapture hang; residual source-close growth is approximately one event per cycle. Pre-fix retained-event stacks point to capture-item and COM remoting allocation, not encoder allocation. Owned GPU encoder input, FPS restart, resize, readback and duration controls are stable. Hardware activation ownership now explicitly shuts down activated transforms; resource acceptance remains open. Capture-only Stop with the source open passes 100 cycles in each of Debug/Release with stable handle counts. Shared v2 command/server-event validators now have native/TypeScript fixtures, plus atomic native room/directory state caches; see refactor/ROOM-PROTOCOL.md. Next: residual source-close event growth, production capture-owner/session integration, external latency baseline and installer/distribution evidence. Actual driver removal and production session callback-generation barriers are not proven. Normal app media still uses the legacy backend; gaming latency remains unmeasured. Continue through substantial milestones without stopping at every intermediate commit.
