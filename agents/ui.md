...
- Worker signaling is now the default Internet path in the UI. The app uses the built-in Worker URL `https://screenshare-signaling.bit-yeet.workers.dev`; users only see Room and local Port controls, and copied secure room links are `screenshare-room-v1;room=...;key=...`. The UI does not ask users to paste the Worker URL.
...
- Worker room sharing uses an automatic hidden room key carried by the room link/session flow. The signaling Worker must not receive that secret. Manual invite fallback still uses the visible Access code or explicit plaintext choice.
