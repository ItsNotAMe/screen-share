...
- `AppSessionBackend` converts the first layer of live engine telemetry into typed `SessionEvent` status snapshots. The UI uses those events for Share viewer rows and the Live/Disconnected indicator instead of parsing `viewer_*` and media-counter log fields itself.
