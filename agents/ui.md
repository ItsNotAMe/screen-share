...
- CLI stop files and runtime resolution control files now flow through `src/core/SessionRuntimeControl.*`; the process adapter still writes files, but the in-process backend should swap in memory-backed stop/settings requests.
