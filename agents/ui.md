...
- Security UX direction: normal room sharing should be encrypted by default with a hidden generated room key. Do not expose "access code" as the default user chore; a visible room password can be an optional extra lock later.
...
- Worker signaling is now the default Internet path in the UI. Share shows Server, Room, and Port, then copies a `screenshare-room-v1;server=...;room=...;port=...` room link. Watch can paste that link to fill Server, Room, and Port. The engine commands are `--share-room PORT --signal-server URL --signal-room ROOM` and `--watch PORT --signal-server URL --signal-room ROOM`.
...
- Current UI invite creation still uses the shared Access code or explicit plaintext choice. The next room flow should replace that with an automatic hidden room key carried by the invite/session flow; the signaling Worker must not receive that secret.
