...
- Internet/NAT fields are a transitional bridge over the CLI invite flow: Create room generates/copies one sharer room invite, Join room pastes that invite and sends probes from the receive socket, and Watch can also create/copy a My invite response. Multiple watchers can paste the same room invite; Share fans out to every valid watcher probe it sees. If Share has a Friend invite response, it uses that as `--share` while still passing its own room invite as `--local-invite`. This keeps the one-invite path for LAN/VPN/reachable-NAT while supporting one blocked NAT fallback without a server.
...
- The main UI should not expose per-watcher NAT invite rows. The current room model is one shared room invite; future room work should add per-viewer connection state after real testing shows what status users need.
