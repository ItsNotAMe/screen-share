...
- Worker signaling is now the default Internet path in the UI. The app uses the built-in Worker URL `https://screenshare-signaling.bit-yeet.workers.dev`; users only see Room and local Port controls, and copied room links are `screenshare-room-v1;room=...`. The UI does not ask users to paste the Worker URL.
