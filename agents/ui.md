...
- Internet/NAT fields are a transitional bridge over the CLI invite flow: the default UI path is now one sharer-owned room invite. Create room generates/copies the sharer's invite, and Join room pastes that invite and sends probes from the receive socket. This works for LAN/VPN/reachable-NAT; the latest real NAT report showed endpoint-filtering can block the one-link path before Share sees any probe.
