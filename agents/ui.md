...
- Internet/NAT fields are a transitional bridge over the CLI invite flow: Create room generates/copies one sharer room invite, Join room pastes that invite and sends probes from the receive socket, and Watch can also create/copy a My invite response. Multiple watchers can paste the same room invite; Share fans out to every valid watcher probe it sees. If strict NAT blocks some probes, paste those watchers' My invite responses into Share's Watcher invites field; the engine sends outward to those watcher endpoints through the same sender socket/local port.
