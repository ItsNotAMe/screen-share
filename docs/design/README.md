...
Settings only. Profile owns nickname editing; do not duplicate that field under Updates.
...
Create provides nickname, room name, visibility, optional password, viewer limit,
...
Host shows source, sharing status, audio and per-viewer requests. Allow/deny/revoke
names the specific capability and peer. Include release-all and the existing panic
