...
public:
    ...
    explicit MfVideoDecoderFactory(bool preferHardware = false, DeviceFactory deviceFactory = {}, std::shared_ptr<DiagnosticHistory> diagnostics = {})
        : preferHardware_(preferHardware), deviceFactory_(std::move(deviceFactory)), diagnostics_(std::move(diagnostics)) {}
