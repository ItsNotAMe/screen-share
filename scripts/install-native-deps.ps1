...
$lock = Get-Content (Join-Path $repoRoot 'cmake/dependencies/native-dependencies.json') -Raw | ConvertFrom-Json
function Invoke-Checked {
    param([string]$Program, [string[]]$Arguments)
    & $Program @Arguments
    if ($LASTEXITCODE -ne 0) { throw "$Program failed with exit code $LASTEXITCODE" }
}
$environmentRoot = Join-Path $DependencyRoot 'dependency-tools'
...
$headers = Join-Path $DependencyRoot 'Vulkan-Headers'
